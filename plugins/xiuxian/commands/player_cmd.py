...
# 帮助
help_cmd = xiuxian_command("灵修帮助", aliases={"修仙帮助", "修仙说明", "帮助"}, priority=5, block=True)
gu_help_cmd = xiuxian_command("蛊修帮助", aliases={"蛊修说明"}, priority=5, block=True)
...
@help_cmd.handle()
async def handle_ling_help(bot: Bot, event, args: Message = CommandArg()):
    ...
    help_text = (
        ...
        "━━━━━━━━━━━━━━━\n"
        "🧘 修炼突破\n"
        "  · 闭关 洞府     → 开始挂机（可选：洞府/灵脉/妖兽森林等），挂机可获得修为+灵石\n"
        "  · 出关          → 结算挂机收益（修为、灵石、功法熟练度）\n"
        "  · 突破          → 修为满后冲击下一境界\n"
        "  · 突破 用破境丹 → 服用破境丹再突破，成功率更高\n"
        "  · 突破需消耗药材+丹药，可到对应等级地图刷取：筑基(洞府/灵脉)、金丹(灵脉)、元婴(妖兽森林)、化神(妖兽森林)、炼虚~飞升(古神药园)；也可等「突破商人」现身坊市直接购买\n"
        "  · 世界          → 查看当前世界事件/天气/秘境状态\n"
        "  · 触发事件 上古秘境开启 → 管理员手动触发世界事件（发送「触发事件 无」清除）\n"
        "  · 召唤商人      → 管理员手动召唤突破商人（专售突破大境界药材/丹药）\n"
        "━━━━━━━━━━━━━━━\n"
        "🗺️ 探索获取资源\n"
        "  · 探索 洞府     → 常开地图有境界限制：洞府/灵脉（炼气~筑基）、妖兽森林（金丹+）、古神药园（化神+）\n"
        "  · 探索 妖兽森林 → 高收益，可能遇险（金丹境界开放）\n"
...
@gu_help_cmd.handle()
async def handle_gu_help(bot: Bot, event, args: Message = CommandArg()):
    group_id = await require_game(gu_help_cmd, event)

    help_text = (
        "🐛 【蛊修·指令大全】\n"
        "━━━━━━━━━━━━━━━\n"
        "🧿 蛊修入门\n"
        "  · 我要修蛊      → 创建蛊修角色（空窍资质：甲＞乙＞丙＞丁＞无）\n"
        "  · 蛊修          → 查看蛊修面板（境界/真元/蛊虫/饥饿）\n"
        "  · 资质决定：空窍容量、真元恢复、承载蛊虫数、修炼上限\n"
        "    丙等以下难上五转，五转是凡人顶点；乙/甲资质方可渡劫成仙\n"
        "━━━━━━━━━━━━━━━\n"
        "🌀 境界修炼\n"
        "  · 采气 <地点>   → 修炼：恢复真元 + 参悟 + 灵石（境界越高、地点越好收益越高）\n"
        "    常驻地点需境界解锁：洞府/灵脉（一转~二转）、妖兽森林（三转+）、古神药园（五转+）\n"
        "    限时地点（秘境/灵药谷等）需世界事件开启\n"
        "  · 蛊修突破      → 修为满后突破小境界/大境界（「蛊修突破 用破境丹」提高成功率）\n"
        "  · 五转巅峰渡劫  → 渡劫成仙（六转起真元化为仙元，寿命大增）\n"
        "━━━━━━━━━━━━━━━\n"
        "🐛 蛊虫\n"
        "  · 寻蛊          → 野外捕捉蛊虫（仙蛊全服唯一）\n"
        "  · 蛊养 1        → 喂食蛊虫（按蛊种消耗妖丹/灵泉/玄铁/千年灵参），久不喂养会饿死\n"
        "  · 炼蛊 1 2 3    → 多只蛊虫合炼更高转蛊（有失败损毁概率）\n"
        "  · 用蛊 1        → 消耗真元催动蛊虫，获得灵石/参悟/物品\n"
        "  · 运蛊          → 查看当前可用的杀招搭配\n"
        "  · 杀招 <id>     → 释放多蛊组合杀招（狂攻/铁壁/百毒/生死/灭世），威力远超单蛊\n"
        "━━━━━━━━━━━━━━━\n"
        "🧿 蛊修战力\n"
        "  · 战力由 境界 + 蛊虫转数 决定，仙蛊翻倍，蛊仙整体提升\n"
        "  · 可参与 PK、大乱斗、讨伐 Boss（共用的指令见「灵修帮助」）\n"
        "━━━━━━━━━━━━━━━\n"
        "💡 小贴士\n"
        "  · 商城/坊市/背包/赠送/排行榜 为通用指令，发送「灵修帮助」查看\n"
        "  · 商城丹药蛊修同样有效：修为丹→参悟、气运/回血/复活丹照常生效、喂养丹→喂蛊、破境丹→蛊修突破加成\n"
        "  · 蛊虫记得及时喂养，不然会饿死！"
    )
    await reply_finish(gu_help_cmd, event, help_text)
