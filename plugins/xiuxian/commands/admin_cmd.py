...
async def handle_game_on(bot: Bot, event, args: Message = CommandArg()):
    ...
    if not group_id:
        await game_on_cmd.finish("请在群聊中使用「开启修仙」")

    if event.user_id != config.admin_qq:
        await game_on_cmd.finish("⛔ 只有管理员可以操作修仙功能开关")

    if db.set_game_enabled(group_id, True):
        logger.info(f"群 {group_id} 修仙功能已开启（操作者 {event.user_id}）")
        await game_on_cmd.finish("✅ 本群修仙功能已开启，道友们可以开始修炼了！")
    await game_on_cmd.finish("操作失败，请稍后再试")
...
async def handle_game_off(bot: Bot, event, args: Message = CommandArg()):
    group_id = require_group(event)
    if not group_id:
        await game_off_cmd.finish("请在群聊中使用「关闭修仙」")

    if event.user_id != config.admin_qq:
        await game_off_cmd.finish("⛔ 只有管理员可以操作修仙功能开关")

    if db.set_game_enabled(group_id, False):
        logger.info(f"群 {group_id} 修仙功能已关闭（操作者 {event.user_id}）")
        await game_off_cmd.finish("⛔ 本群修仙功能已关闭，玩家数据将保留，管理员发送「开启修仙」可重新启用")
    await game_off_cmd.finish("操作失败，请稍后再试")
