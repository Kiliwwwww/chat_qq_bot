from nonebot import on_command
from nonebot.adapters.onebot.v11 import MessageEvent

help_cmd = on_command("help", aliases={"帮助"}, priority=5, block=True)


@help_cmd.handle()
async def handle_help(event: MessageEvent):
    """显示帮助信息"""
    help_text = """可用命令：
/help 或 /帮助 - 显示此帮助
/reset 或 /重置对话 - 重置当前对话历史
/闭嘴 - 让bot静默5分钟（管理员）
/禁言 <QQ号/@某人> [分钟数] - 禁言指定用户，默认5分钟（bot管理员/群管理员）
/settings <QQ号> 或 /设置 - 管理用户白名单（管理员）
/groupsettings <群号> 或 /群设置 - 管理群白名单（管理员）
/setkey <一句话> 或 /设置关键词 - 添加提示词（管理员）
/setkey 或 /设置关键词 - 查看所有提示词（管理员）
/setkey del <ID> 或 /设置关键词 del <ID> - 删除提示词（管理员）
/邻家大姐姐人格 - 切换到邻家大姐姐人格
/雌小鬼人格 - 切换回默认人格
/贴表情 <QQ号> - 给指定用户消息贴随机表情（管理员）
/取消贴表情 <QQ号> - 取消给指定用户贴的表情（管理员）
/贴表情all <QQ号> - 给指定用户在所有群贴🐒表情（管理员，支持私聊）
/取消贴表情all <QQ号> - 取消给指定用户在所有群贴🐒表情（管理员，支持私聊）
/全体贴表情 - 给群内所有人消息随机贴表情（管理员）
/取消全体贴表情 - 取消全体贴表情（管理员）
/欢迎语 <内容> - 设置新成员入群欢迎语（管理员）
/欢迎语 - 查看当前群欢迎语（管理员）
/开启群撤回 - 开启群广告消息自动撤回（管理员）
/关闭群撤回 - 关闭群广告消息自动撤回（管理员）
/广告词 - 查看广告关键词列表（管理员）
/广告词 <关键词> - 添加广告关键词（管理员）
/广告词 del <ID> - 删除广告关键词（管理员）
/撤回状态 - 查看群撤回功能状态（管理员）
/排行榜 或 /ranking - 查看群发言排行榜（管理员）
/weibo <UID> - 获取微博用户最新动态（私聊）
/sendweibo <UID> <群号> - 发送微博图片到指定群（私聊）

---娱乐互动---
/今日老婆 - 抽取今日老婆（群聊，每天一次）
/原神家庭 - 抽取家庭成员（群聊，需先抽今日老婆，每天最多3个）
/每日猪猪 - 抓猪猪并查看今日运势（群聊，每天一次）
/今日老婆配置 <QQ号> <配对QQ号> - 固定某人的今日老婆（私聊）

/添加ai服务 <名称> <api_key> <base_url> <model> - 添加AI服务（管理员）
/ai服务 - 查看所有AI服务列表
/切换ai服务 <id> - 切换到指定AI服务（管理员）
/删除ai服务 <id> - 删除指定AI服务（管理员）

---知识库管理（私聊）---
/kb_add <群号> <知识库ID> - 开启群知识库功能（管理员）
/kb_del <群号> - 关闭群知识库功能（管理员）
/kb_list - 查看知识库群配置列表（管理员）
/kb_upload - 手动上传今天的聊天记录（管理员）
/kb_upload <群号> [日期] - 上传指定群聊天记录
/kb_summary - 生成今天的聊天记录总结并上传（管理员）
/kb_summary <群号> [日期] - 生成指定群聊天记录总结并上传
/kb_sync <群号> [日期] - 同步上传聊天记录并生成总结（管理员，日期默认今天）
/stats 或 /统计 - 查看功能使用统计（检索/上传/解析/AI请求次数）
/send <群号> <内容> - 向指定QQ群发送消息（管理员）
/朗读 <内容> 或 /read <内容> - 用女声朗读指定内容并发送语音
/发语音 <群号> <内容> [语气指令] - 用语音朗读内容并发送到指定群（管理员；内容含空格时用 | 分隔语气）
/音色 <描述> - 设置全局语气/口音（如「用台湾腔说话」），/音色 默认 恢复（管理员）"""
    await help_cmd.finish(help_text)
