...
class PairingManager:
    ...
    def _get_daily_result_key(self, user_id: int) -> str:
        """获取每日结果的Redis key（包含日期）"""
        today = datetime.now().strftime("%Y%m%d")
        return f"daily_wife:result:{today}:{user_id}"
    ...
    def get_daily_result(self, user_id: int) -> int | None:
        """获取用户今日的老婆结果"""
        if self.redis_client is None:
            return None
        try:
            key = self._get_daily_result_key(user_id)
    ...
    def set_daily_result(self, user_id: int, wife_id: int) -> bool:
        """保存用户今日的老婆结果（24小时后过期）"""
        if self.redis_client is None:
            return False
        try:
